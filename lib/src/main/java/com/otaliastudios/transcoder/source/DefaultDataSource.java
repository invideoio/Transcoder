...
public abstract class DefaultDataSource implements DataSource {
    ...
    private final ArrayList<Long> keyFrameTsUsList = new ArrayList<>();
    @Override
    ...
    @Override
    public ArrayList<Long> getKeyFrameTimestampsUsList() {
        return keyFrameTsUsList;
    }
    ...
    public long requestKeyFrameTimestamps() {
        ...
        if(keyFrameTsUsList.size() > 0) {
            mExtractor.seekTo(keyFrameTsUsList.get(keyFrameTsUsList.size() - 1) + 10001L, MediaExtractor.SEEK_TO_NEXT_SYNC);
        }

        long sampleTime = mExtractor.getSampleTime();

        if (sampleTime == -1 || (keyFrameTsUsList.size() > 0 && sampleTime == keyFrameTsUsList.get(keyFrameTsUsList.size() - 1))) {
            lastKeyFrame = true;
            mExtractor.seekTo(keyFrameTsUsList.get(keyFrameTsUsList.size() - 1) + 10001L, MediaExtractor.SEEK_TO_PREVIOUS_SYNC);
            return -1;
        }
        ...
        while (sampleTime >= 0L && sampleTime != lastSampleTime && count < prefetchCount) {
            if ((mExtractor.getSampleFlags() & MediaExtractor.SAMPLE_FLAG_SYNC) > 0) {
                if (!keyFrameTsUsList.isEmpty() && sampleTime <= keyFrameTsUsList.get(keyFrameTsUsList.size() - 1)) {
                    Collections.sort(keyFrameTsUsList);
                }
                keyFrameTsUsList.add(sampleTime);
            }
        ...
        LOG.i("keyFrameStopCount:" + keyFrameTsUsList);
        return sampleTime;
    }
